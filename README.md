...
A lightweight automation that removes songs from a Spotify playlist once you've listened to them. Useful for temporary queues, Discover Weekly clones, or inbox-style music sorting. Runs on macOS, Linux, and Windows.
...
├── .cache                            # Spotify token cache (autogenerated)
├── main.py                           # Core script
├── com.user.discovercleaner.plist    # launchd LaunchAgent template
└── pyproject.toml
```
